...
import {
  Plus, Trash2, ArrowLeft, Search, User,
  Truck, Check, Loader2, Package, FileText,
} from "lucide-react";
    ...
    <div className="lux-sync warehouse-unified p-6 space-y-6 bg-gradient-to-br from-yellow-50 via-yellow-50 to-amber-50 min-h-screen">
      <div className="space-y-6 w-full">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <ArrowLeft className="h-4 w-4" /> Quay lại danh sách báo giá
          </button>
        </div>
            ...
            <div className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/80 overflow-hidden">
              <div className="flex items-center gap-3 px-6 py-5 border-b border-slate-100 bg-slate-50">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">Chọn khách hàng cần báo giá</p>
                </div>
              </div>
              ...
              <div className="p-5 space-y-5">
                ...
                <div className="space-y-1.5">
                  ...
                  {showCustomerDropdown && filteredCustomers.length > 0 && (
                    <div className="absolute z-50 w-[85%] mt-1 bg-white border border-gray-200 rounded-xl shadow-xl max-h-48 overflow-y-auto ring-1 ring-slate-200/80">
                  ...
                  {selectedCustomer && (
                    <div className="p-3 mt-2 bg-slate-50 border border-slate-100 rounded-xl text-xs space-y-2 text-slate-700">
                ...
                <div className="pt-2 border-t border-slate-100 space-y-1.5">
                  <Label className="text-xs font-semibold uppercase tracking-wider text-gray-500">Ghi chú báo giá</Label>
                  <Textarea
            ...
            <div className="rounded-2xl bg-white shadow-sm ring-1 ring-slate-200/80 overflow-hidden min-h-[400px]">
              <div className="flex items-center justify-between gap-3 px-6 py-5 border-b border-slate-100 bg-slate-50">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-slate-900 leading-snug">Chi tiết mặt hàng</p>
                    <p className="text-xs text-slate-500 mt-0.5">Sản phẩm và số lượng báo giá</p>
                  </div>
                </div>
                <Button
              ...
              {orderItems.length === 0 ? (
                <div className="flex flex-col items-center justify-center px-6 py-16 text-center">
                  ...
                  <p className="mt-2 max-w-sm text-sm leading-6 text-slate-500">Nhấn "Thêm sản phẩm" để lập báo giá.</p>
                </div>
              ) : (
                <>
                    ...
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="border-b border-slate-200 bg-slate-50">
                          ...
                          <th className="h-12 px-4 text-center font-semibold text-slate-600 tracking-wide text-xs uppercase w-28">Số lượng</th>
                          <th className="h-12 px-4 text-right font-semibold text-slate-600 tracking-wide text-xs uppercase w-36">Đơn giá</th>
                          <th className="h-12 px-4 text-right font-semibold text-slate-600 tracking-wide text-xs uppercase w-36">Thành tiền</th>
                  ...
                  <div className="px-6 py-5 bg-slate-50 border-t border-slate-100 space-y-2">
                    ...
                    <div className="flex justify-between text-base pt-3 border-t border-slate-200">
                      <span className="font-bold text-slate-900">Tổng báo giá</span>
      ...
      <Dialog open={showProductDialog} onOpenChange={setShowProductDialog}>
        <DialogContent className="max-w-2xl p-0 overflow-hidden border-none shadow-2xl rounded-2xl" style={{ background: "#faf7f0", color: "#0f172a", outline: "none" }}>
              ...
              <table className="w-full text-sm" style={{ background: "#faf7f0", borderCollapse: "collapse" }}>
                <thead>
                  <tr style={{ background: "#f5efe0" }}>
                    <th className="h-10 px-6 text-left font-semibold tracking-wide text-xs uppercase" style={{ color: "#64748b", borderTop: "1px solid #ede8db", borderBottom: "1px solid #ede8db" }}>Sản phẩm</th>
                    <th className="h-10 px-4 text-right font-semibold tracking-wide text-xs uppercase" style={{ color: "#64748b", borderTop: "1px solid #ede8db", borderBottom: "1px solid #ede8db" }}>Giá bán</th>
                    <th className="h-10 px-4 w-24" style={{ borderTop: "1px solid #ede8db", borderBottom: "1px solid #ede8db" }} />
                  </tr>
                </thead>
                <tbody>
                  {filteredProducts.map((product, idx) => (
                    <tr
                      key={product.id}
                      style={{ background: "#faf7f0", borderBottom: idx < filteredProducts.length - 1 ? "1px solid #ede8db" : "none", cursor: "pointer" }}
