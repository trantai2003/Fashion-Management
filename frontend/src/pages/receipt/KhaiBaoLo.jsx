...
export default function KhaiBaoLo() {
    ...
    // LOGIC KIỂM SOÁT TRẠNG THÁI MỚI
    // Chỉ cho phép chỉnh sửa nếu phiếu đang ở trạng thái Nháp (0)
    const isEditable = detail?.phieu?.trangThai === 0;
    const isCompleted = detail?.phieu?.trangThai === 3;
    const isCancelled = detail?.phieu?.trangThai === 4;
    ...
    async function handleSaveLot() {
        if (!isEditable) {
            return toast.error("Phiếu không còn ở trạng thái nháp, không thể chỉnh sửa");
        }
            ...
            <main className="flex-1">
                ...
                <div className="max-w-7xl mx-auto px-6 py-6 space-y-6">
                    ...
                    <section className="bg-white border rounded-xl p-6 shadow-sm">
                        <h2 className="text-sm font-semibold mb-4 tracking-wider text-gray-500">Thông tin biến thể</h2>
                        <div className="grid md:grid-cols-4 gap-4 text-sm">
                            ...
                            <Info
                                ...
                                className={isEnough ? "text-green-600 font-bold" : "text-red-600 font-bold"}
                            />
                        </div>
                    </section>
                    ...
                    <section className="bg-white border rounded-xl shadow-sm overflow-hidden">
                        <div className="p-4 font-semibold border-b bg-gray-50 flex justify-between">
                            <span>Danh sách lô hàng</span>
                            {!isEditable && <span className="text-xs text-amber-600 font-normal italic">Chế độ xem (Phiếu đã khóa)</span>}
                        </div>
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {lotList.length === 0 ? (
                                    <tr>
                                        <td colSpan={5} className="px-4 py-10 text-center text-gray-400 italic">Chưa có lô nào được khai báo</td>
                                    </tr>
                                ) : (
                                    lotList.map((lo) => (
                                        <tr key={lo.loHangId} 
                    ...
                    {/* FORM THÊM LÔ: Chỉ hiện khi isEditable */}
                    {isEditable ? (
                        ...
                        <div className="p-4 bg-amber-50 border border-amber-200 rounded-xl text-amber-700 text-sm flex items-center gap-2">
                            Phiếu đang ở trạng thái<strong>{detail.phieu.trangThai === 4 ? 'ĐÃ HỦY' : 'ĐÃ KHÓA'}</strong>. Bạn không thể chỉnh sửa lô hàng tại màn hình này.
                        </div>
                    )}
                </div>
...
function Info({ label, value, bold, className }) {
    return (
        <div className="space-y-1">
            ...
            <div className={`${bold ? "font-bold text-gray-900" : "text-gray-700"} ${className || ""}`}>{value}</div>
        </div>
    );
}