// src/pages/attribute/ColorList.jsx
...
import {
    ...
    Eye, Loader2, Palette, ChevronDown, Filter, RefreshCcw,
    Check, AlertCircle, X,
} from "lucide-react";
...
export default function ColorList() {
    ...
    const fetchColors = useCallback(async () => {
        setLoading(true);
        try {
            // Sử dụng filter API với payload rỗng để lấy tất cả
            const res = await mauSacService.filter({ 
                page: 0, 
                size: 1000, 
                filters: [], 
    ...
    // Hàm xử lý xác nhận xóa màu sắc
    const handleConfirmDelete = async () => {
        if (!deleteTarget) return;
        setIsDeleting(true);
        try {
            const res = await mauSacService.delete(deleteTarget.id);
            if (res.status === 200) {
                toast.success(`Đã xóa màu sắc "${deleteTarget.tenMau}"`);
                setColors(prev => prev.filter(s => s.id !== deleteTarget.id));
    ...
    // Lọc và phân trang dữ liệu (Local filtering for better UX if data is not too large)
    const filtered = useMemo(() => colors.filter(item => {
        const matchSearch = !search.trim()
            || item.maMau?.toLowerCase().includes(search.toLowerCase())
            || item.tenMau?.toLowerCase().includes(search.toLowerCase())
            || item.maMauHex?.toLowerCase().includes(search.toLowerCase());
        return matchSearch;
    }), [colors, search]);
    ...
    return (
        <div className="space-y-6">
                ...
                <Card className="border-0 shadow-lg hover:shadow-xl transition-all duration-200 bg-white">
                    <CardContent className="p-6">
                        <div className="flex items-center justify-between">
                            <div>
                                <p className="text-sm font-medium text-gray-500">Tổng màu sắc</p>
                                <p className="text-2xl font-bold text-gray-900 mt-1">{colors.length}</p>
                            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div className="space-y-2 md:col-span-3">
                            ...
                            <div className="relative">
                                ...
                                <Input
                                    placeholder="Mã, tên màu hoặc mã HEX..."
            ...
            <div className="flex justify-end">
                <button
                    type="button"
                    onClick={() => navigate("/attribute/color/new")}
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-slate-200 bg-slate-50">
                                ...
                                <th className="h-12 px-4 text-left font-semibold text-slate-600 tracking-wide text-xs uppercase">Mã màu</th>
                                <th className="h-12 px-4 text-left font-semibold text-slate-600 tracking-wide text-xs uppercase">Tên màu</th>
                                <th className="h-12 px-4 text-left font-semibold text-slate-600 tracking-wide text-xs uppercase">HEX Visual</th>
                                    ...
                                    <tr key={item.id} className="transition-colors duration-150 hover:bg-yellow-50/50">
                                        ...
                                        <td className="px-4 py-3.5 align-middle">
                                            <div className="flex items-center justify-center gap-1">
                                                <button
                                                    onClick={() => navigate(`/attribute/color/view/${item.id}`)}
                                                ...
                                                <button
                                                    onClick={() => navigate(`/attribute/color/${item.id}`)}
                    ...
                    >
                        ...
                        <div className="px-6 pb-4">
                            <p className="text-sm text-gray-700">
                                ...
                                <span className="font-bold text-gray-900">"{deleteTarget.tenMau}"</span>?
                            </p>
                        </div>
